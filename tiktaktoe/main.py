...
# to mark how put their sign where
def get_text(i, j, gb, l1, l2):
    global sign
    if board[i][j] == ' ':
        # lets exact player make their move
        if sign % 2 == 0:
...
# if it is that mean thay made all moves and nobady won, so its a tie
def isfull():
    flag = True
    for i in board:
        if(i.count(' ') > 0):
...
# Decide the next move of system (PC)
def pc():
    possiblemove = []
